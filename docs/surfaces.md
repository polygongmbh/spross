...
- **Trainers**: the Training card offers TWO entries, Zahlen and Buchstaben. Clock,
  ...
  (reverse, fast, mix) are how it is played. Which of them a learner may pick is derived
...
- **Zahlen overview**: the numbers entry opens a page, not a run — reference first,
  options second, start last. The reference table is GENERATED by `Trainer.reference`
  from the packs the drill grades against, so the page cannot claim one reading and mark
  another; kern names the bands and the app words their headings. Beside it sit two to
  ...
  never open is a lie.
- **Letter drill & alphabet sheet**: the Training card shows when slots OR an
  alphabet exist for the target (the second predicate is catalog file presence, recomputed
