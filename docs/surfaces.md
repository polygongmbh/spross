...
- **Zahlen overview**: the numbers entry opens a page, not a run — reference first,
  ...
  and the in-run "?" raises the very same table, for every language — a look-up while the
...
- **Buchstaben overview**: the letters entry opens the same shape as the numbers one —
  the alphabet table first, the drill's stages second, start last. The Training card shows
...
- **Letter drill**: the drill asks everything by ear: letter
