...
Moved to `date-readings.md` (commit C1, landed) —
the sibling page of `clock-registers.md` and `number-forms.md`.
Every ⚠ there wants a native ruling; the drill ships the reading named first.
The load-bearing consequences for this plan:
the day of month is GENERATED by the pack, never authored;
Ukrainian needs a `dateForm` on its months and a genitive `dateDay` override, and carries no rung 6.
