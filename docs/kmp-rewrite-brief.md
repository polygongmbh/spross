...
- **`feminineOf` (nouns):** the feminine sibling of its base. **Direction-aware
  emission** — emit a `Card` for a direction only when the ANSWER (produced) language
  has a distinct feminine realization (de always; uk where distinct; sw never). You
  only drill producing a genuinely distinct form; otherwise no card (NOT an
  autogenerated answer). On the prompt side, if the prompt language has no distinct
  ...
  in the graded answer. Card icon = base emoji + ♀ (feminine concepts carry no emoji).
  A future non-German pair (sw↔uk) should suppress these siblings entirely.
