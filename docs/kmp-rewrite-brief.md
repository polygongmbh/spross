...
- **`feminineOf` (nouns):** the feminine sibling of its base, and an **exception to
  join+skip coverage** — it ALWAYS yields a `Card` in every pair. The de side is the
  distinct German feminine (the payload). The target side is the authored realization
  where the target distinguishes (de↔uk `Lehrerin↔вчителька`), else **autogenerated** =
  the base concept's target text + this concept's emoji as a ♀ marker (de↔sw
  `Lehrerin↔mwalimu 👩`; uk epicene `колега 👩`). The marker keeps the feminine card
  distinct from the base card in the target→de direction (same target text, different
  expected German). The base card is unchanged.
