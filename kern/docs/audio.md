...
- **Lookup is keyed by the MATCHED SPOKEN FORM, never by the slug.**
  ...
  Three sections feed them: `words`, the alphabet's slugless `texts`, and the calendar's
  `calendar` — the weekday and month names, which no concept covers and which the dates drill
  therefore had nothing but the device voice to say.
  One index, so a drill hears a recorded `Montag` through the very lookup a card uses,
  and no surface needs a route of its own.
