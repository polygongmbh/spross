...
class Catalog internal constructor(
     ...
     */
    fun dateNames(lang: Language): DateCalendar? = dateCalendars[lang]
     ...
     * rule ([phraseTemplates]), for the frame's reason. Both calendars carry the full
     * seven and twelve by parse, so the join is total and nothing downstream asks whether
     * a name exists.
     */
    fun dateDrillContent(source: Language, target: Language): DateDrillContent? {
        require(source != target) { "source == target ($source)" }
