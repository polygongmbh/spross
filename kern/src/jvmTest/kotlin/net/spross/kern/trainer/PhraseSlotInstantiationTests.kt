...
/**
 * Hand-picked instantiations of the real frames — one language pair per section, each
 * asserting the exact prompt, display and accepted set a slot renders into.
 *
 * These are the worked examples; the rules they are examples OF (casing, notes, variant
 * frames, the frame × slot expansion, join shape) are [PhraseSlotTests].
 */
class PhraseSlotInstantiationTests {
    ...
    fun ukrainianYearDictation() {
        ...
        assertEquals("Повторіть, будь ласка, дату: тисяча дев'ятсот сімдесят вісім.", task.display)
        assertTrue("Повторіть, будь ласка: одна тисяча дев'ятсот сімдесят вісім." in task.accepted)
    }
