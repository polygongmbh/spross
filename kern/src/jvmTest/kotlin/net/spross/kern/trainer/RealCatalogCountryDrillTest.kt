...
class RealCatalogCountryDrillTest {
    ...
    fun everyPairOpensOnItsOwnCountries() {
        for ((source, target) in pairs()) {
            ...
            assertTrue(
                ...
                "$source→$target: Sprosse 1 has nothing to ask",
            )
        }
    }
     ...
     * real pair loses its opening Sprosse to that (which is what the fallback would rescue —
    ...
    /**
     * Two hundred draws a Sprosse, both directions, every pair: what this catches is a pool
