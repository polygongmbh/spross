 ...
 * past the half hour.
 */
class NumbersSpanishTests {

    private fun display(n: Long) = Numbers.number(n, "es").display
    private fun accepted(n: Long) = Numbers.number(n, "es").accepted
    ...
    fun yearsAreReadAsPlainCardinals() {
        val y1978 = Numbers.year(1978, "es")
        ...
        assertEquals("dos mil cinco", Numbers.year(2005, "es").display)
        assertEquals("dos mil", Numbers.year(2000, "es").display)
        assertEquals("mil novecientos", Numbers.year(1900, "es").display)
    }

    @Test
    fun clockAgreesWithTheNamedHour() {
        assertEquals("son las dos de la tarde", Numbers.clock(14, 0, "es").display)
        assertEquals("es la una de la tarde", Numbers.clock(13, 0, "es").display)
        assertEquals("son las dos y media de la tarde", Numbers.clock(14, 30, "es").display)
        assertEquals("es la una y cuarto de la tarde", Numbers.clock(13, 15, "es").display)
        assertEquals("son las tres menos cuarto de la tarde", Numbers.clock(14, 45, "es").display)
        // The copula follows the hour the reading names, not the clock's hour.
        assertEquals("es la una menos cuarto de la tarde", Numbers.clock(12, 45, "es").display)
        assertEquals("son las tres menos veinticinco de la tarde", Numbers.clock(14, 35, "es").display)
        assertEquals("son las dos y diecisiete de la tarde", Numbers.clock(14, 17, "es").display)
    }

    @Test
    fun clockAcceptsTheTypedVariants() {
        val half = Numbers.clock(14, 30, "es").accepted
    ...
    fun theDayPartFollowsTheNamedHourAndStaysOptional() {
        assertEquals("son las cinco de la madrugada", Numbers.clock(5, 0, "es").display)
        assertEquals("son las cinco de la tarde", Numbers.clock(17, 0, "es").display)
        // 19:45 reads as eight o'clock, and eight is de la noche.
        assertEquals("son las ocho menos cuarto de la noche", Numbers.clock(19, 45, "es").display)
        assertTrue("son las ocho menos cuarto de la tarde" in Numbers.clock(19, 45, "es").accepted)
        // Counting DOWN to noon is still morning: never "del día".
        assertEquals("son las doce menos cuarto de la mañana", Numbers.clock(11, 45, "es").display)
        ...
        for (bare in listOf("son las cinco menos cuarto", "cinco menos cuarto", "las cinco menos cuarto")) {
            assertTrue(bare in Numbers.clock(16, 45, "es").accepted, bare)
        }
    }
    ...
    fun theAmericanCountdownIsAccepted() {
        val toThree = Numbers.clock(14, 40, "es").accepted
        ...
        val toOne = Numbers.clock(12, 45, "es").accepted
        ...
        assertTrue(Numbers.clock(14, 40, "es").accepted.none { " a las " in it })
    }

    /** Timetables, news and announcements run 0–23 and name no part of the day. */
    @Test
    fun theTimetableRegisterIsAccepted() {
        assertTrue("son las catorce treinta" in Numbers.clock(14, 30, "es").accepted)
        assertTrue("son las catorce horas treinta minutos" in Numbers.clock(14, 30, "es").accepted)
        assertTrue("son las dieciocho treinta y cinco" in Numbers.clock(18, 35, "es").accepted)
        assertTrue("son las veintiuna horas" in Numbers.clock(21, 0, "es").accepted)
        // One o'clock stays singular even here.
        assertTrue("es la una treinta" in Numbers.clock(1, 30, "es").accepted)
        assertTrue(Numbers.clock(1, 30, "es").accepted.none { it.startsWith("son las una") })
    }

    /** A minute is counted with its noun, never with a bare "uno". */
    @Test
    fun oneMinuteIsCountedWithTheNoun() {
        assertEquals("son las dos y un minuto de la tarde", Numbers.clock(14, 1, "es").display)
        assertEquals("son las tres menos un minuto de la tarde", Numbers.clock(14, 59, "es").display)
        assertTrue("un minuto para las tres" in Numbers.clock(14, 59, "es").accepted)
        // An apocopating count keeps its noun apocopated too.
        assertTrue("son las dos y veintiún minutos" in Numbers.clock(14, 21, "es").accepted)
        for (task in listOf(Numbers.clock(14, 1, "es"), Numbers.clock(14, 21, "es"), Numbers.clock(14, 59, "es"))) {
    ...
    fun middayAndMidnightAreNamed() {
        val midnight = Numbers.clock(0, 0, "es")
        ...
        val noon = Numbers.clock(12, 0, "es")
        ...
        assertEquals("son las doce y media de la noche", Numbers.clock(0, 30, "es").display)
    }
}
