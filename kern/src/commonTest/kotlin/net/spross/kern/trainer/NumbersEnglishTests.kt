 ...
 */
class NumbersEnglishTests {

    private fun display(n: Long) = Numbers.number(n, "en").display
    private fun accepted(n: Long) = Numbers.number(n, "en").accepted
    ...
    fun yearsSplitPairReadingFromFullCardinal() {
        val y1978 = Numbers.year(1978, "en")
        ...
        val y2005 = Numbers.year(2005, "en")
        ...
        assertEquals("twenty nineteen", Numbers.year(2019, "en").display)
        assertEquals("nineteen hundred", Numbers.year(1900, "en").display)
        assertEquals("two thousand", Numbers.year(2000, "en").display)

        val y1905 = Numbers.year(1905, "en")
    ...
    fun clockReadsPastAndTo() {
        assertEquals("two o'clock", Numbers.clock(14, 0, "en").display)
        assertEquals("quarter past two", Numbers.clock(14, 15, "en").display)
        assertEquals("half past two", Numbers.clock(14, 30, "en").display)
        assertEquals("quarter to three", Numbers.clock(14, 45, "en").display)
        assertEquals("twenty-five to three", Numbers.clock(14, 35, "en").display)
        assertEquals("five past two", Numbers.clock(14, 5, "en").display)
        // Off the five-minute grid a clock is read out, not counted from the hour.
        assertEquals("two seventeen", Numbers.clock(14, 17, "en").display)
        assertTrue("seventeen past two" in Numbers.clock(14, 17, "en").accepted)
        assertEquals("eleven fifty-nine", Numbers.clock(23, 59, "en").display)
        // The prompt is the only place digits appear.
        assertEquals("14:35", Numbers.clock(14, 35, "en").prompt)
    }
    ...
    fun clockAcceptsTheDayPartAndTheTwentyFourHourReading() {
        val quarterToFive = Numbers.clock(16, 45, "en").accepted
        ...
        assertTrue("two o'clock in the morning" in Numbers.clock(2, 0, "en").accepted)
        assertTrue("ten past eleven at night" in Numbers.clock(23, 10, "en").accepted)
        assertTrue("ten after two" in Numbers.clock(14, 10, "en").accepted)
        assertTrue("oh nine hundred hours" in Numbers.clock(9, 0, "en").accepted)
        assertTrue("twenty-two oh five" in Numbers.clock(22, 5, "en").accepted)
        // "of" is one edit from the digital "oh", so a numeric "ten of three" would
        // grade correct at 3:10 — only the quarter takes it.
        assertTrue(Numbers.clock(14, 50, "en").accepted.none { " of " in it })
        // The calque the 24-hour prompt invites stays wrong, at every hour past twelve.
        assertTrue(Numbers.clock(14, 0, "en").accepted.none { "fourteen o'clock" in it })
        assertTrue(Numbers.clock(16, 0, "en").accepted.none { "sixteen o'clock" in it })
    }

    /** Counting off midnight and noon by name, either side of them. */
    @Test
    fun midnightAndNoonAnchorTheHalfHourEitherSide() {
        assertTrue("quarter past midnight" in Numbers.clock(0, 15, "en").accepted)
        assertTrue("half past midnight" in Numbers.clock(0, 30, "en").accepted)
        assertTrue("one minute to midnight" in Numbers.clock(23, 59, "en").accepted)
        assertTrue("quarter to noon" in Numbers.clock(11, 45, "en").accepted)
        assertTrue("ten past noon" in Numbers.clock(12, 10, "en").accepted)
        assertTrue("ten past midday" in Numbers.clock(12, 10, "en").accepted)
        // Accepted only — the ordinary reading is what the reveal teaches.
        assertEquals("half past twelve", Numbers.clock(0, 30, "en").display)
    }

    @Test
    fun clockAcceptsTheTypedVariants() {
        val quarter = Numbers.clock(14, 15, "en").accepted
        ...
        val half = Numbers.clock(14, 30, "en").accepted
        assertTrue("two thirty" in half)
        assertTrue("half two" in half)

        val toThree = Numbers.clock(14, 45, "en").accepted
        assertTrue("a quarter to three" in toThree)
        assertTrue("two forty-five" in toThree)
        assertTrue("two forty five" in toThree)

        assertTrue("two oh five" in Numbers.clock(14, 5, "en").accepted)
        assertTrue("two" in Numbers.clock(14, 0, "en").accepted)
    }

    @Test
    fun midnightAndNoonAreNamed() {
        val midnight = Numbers.clock(0, 0, "en")
        ...
        val noon = Numbers.clock(12, 0, "en")
