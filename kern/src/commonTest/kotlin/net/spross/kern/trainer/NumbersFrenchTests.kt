 ...
 */
class NumbersFrenchTests {

    private fun display(n: Long) = Numbers.number(n, "fr").display
    private fun accepted(n: Long) = Numbers.number(n, "fr").accepted
    ...
    fun yearsTakeTheDateAndHundredCountingSpellings() {
        val y1978 = Numbers.year(1978, "fr")
        ...
        assertEquals("mille neuf cents", Numbers.year(1900, "fr").display)
        assertTrue("dix-neuf cents" in Numbers.year(1900, "fr").accepted)
        assertEquals("deux mille cinq", Numbers.year(2005, "fr").display)
        // Past the thousand nobody counts hundreds, and "mil" is a date spelling only.
        assertTrue(Numbers.year(2005, "fr").accepted.none { "cent" in it || it.startsWith("mil ") })
    }

    @Test
    fun theClockReadsBareAndAgreesWithItsHour() {
        assertEquals("deux heures", Numbers.clock(14, 0, "fr").display)
        assertEquals("une heure", Numbers.clock(13, 0, "fr").display)
        assertEquals("deux heures et quart", Numbers.clock(14, 15, "fr").display)
        assertEquals("deux heures et demie", Numbers.clock(14, 30, "fr").display)
        assertEquals("trois heures moins le quart", Numbers.clock(14, 45, "fr").display)
        assertEquals("trois heures moins vingt", Numbers.clock(14, 40, "fr").display)
        assertEquals("deux heures dix", Numbers.clock(14, 10, "fr").display)
        // Off the five-minute grid the minute is read out, never counted back.
        assertEquals("deux heures trente-sept", Numbers.clock(14, 37, "fr").display)
        // The minute is feminine, so a count ending in one agrees with it.
        assertEquals("deux heures une", Numbers.clock(14, 1, "fr").display)
        assertEquals("deux heures vingt et une", Numbers.clock(14, 21, "fr").display)
        assertEquals("trois heures moins une", Numbers.clock(14, 59, "fr").display)
    }

    @Test
    fun theClockAcceptsTheTypedVariants() {
        val quarter = Numbers.clock(14, 15, "fr").accepted
        ...
        val toThree = Numbers.clock(14, 45, "fr").accepted
        ...
        assertTrue("deux heures trente" in Numbers.clock(14, 30, "fr").accepted)
        assertTrue("il est deux heures" in Numbers.clock(14, 0, "fr").accepted)
    }

    /** The part of the day belongs to the hour the reading NAMES, and is optional. */
    @Test
    fun theDayPartFollowsTheNamedHour() {
        assertTrue("deux heures de l'après-midi" in Numbers.clock(14, 0, "fr").accepted)
        assertTrue("deux heures du matin" in Numbers.clock(2, 0, "fr").accepted)
        assertTrue("deux heures de la nuit" in Numbers.clock(2, 0, "fr").accepted)
        assertTrue("onze heures du soir" in Numbers.clock(23, 0, "fr").accepted)
        // 19:45 names eight in the evening, while the clock still says seven.
        assertTrue("huit heures moins le quart du soir" in Numbers.clock(19, 45, "fr").accepted)
        // Nothing is compulsory: every reading grades bare too.
        assertTrue("huit heures moins le quart" in Numbers.clock(19, 45, "fr").accepted)
        // Noon and midnight ARE the half of the day, so no part is hung on them.
        assertTrue(Numbers.clock(12, 0, "fr").accepted.none { "matin" in it || "après" in it })
    }

    /** Timetables, news and announcements run 0–23 and name no part of the day. */
    @Test
    fun theTimetableRegisterIsAcceptedAndGlossed() {
        assertEquals("aussi : quatorze heures trente", Numbers.clock(14, 30, "fr").gloss)
        assertTrue("il est quatorze heures trente" in Numbers.clock(14, 30, "fr").accepted)
        assertTrue("zéro heure trente" in Numbers.clock(0, 30, "fr").accepted)
        assertTrue("vingt et une heures" in Numbers.clock(21, 0, "fr").accepted)
        assertTrue("dix-huit heures" in Numbers.clock(18, 0, "fr").accepted)
        // Below thirteen the two registers coincide, so the reveal has nothing to add.
        assertEquals(null, Numbers.clock(3, 5, "fr").gloss)
    }

    /** midi and minuit take no "heures", and being masculine they take "et demi". */
    @Test
    fun noonAndMidnightAreNamedAndTakeTheMasculineHalf() {
        assertEquals("midi", Numbers.clock(12, 0, "fr").display)
        assertEquals("minuit", Numbers.clock(0, 0, "fr").display)
        assertEquals("midi et demi", Numbers.clock(12, 30, "fr").display)
        assertEquals("minuit et demi", Numbers.clock(0, 30, "fr").display)
        assertEquals("midi moins le quart", Numbers.clock(11, 45, "fr").display)
        assertEquals("minuit moins le quart", Numbers.clock(23, 45, "fr").display)
        assertTrue("il est midi" in Numbers.clock(12, 0, "fr").accepted)
        assertTrue("zéro heure" in Numbers.clock(0, 0, "fr").accepted)
        assertTrue("douze heures" in Numbers.clock(12, 0, "fr").accepted)
        for (task in listOf(Numbers.clock(12, 30, "fr"), Numbers.clock(0, 30, "fr"))) {
