 ...
 */
class NumbersItalianTests {

    private fun display(n: Long) = Numbers.number(n, "it").display
    private fun accepted(n: Long) = Numbers.number(n, "it").accepted
    ...
    fun yearsAreReadAsPlainCardinals() {
        val y1978 = Numbers.year(1978, "it")
        assertEquals("millenovecentosettantotto", y1978.display)
        assertEquals(listOf("millenovecentosettantotto"), y1978.accepted)
        assertEquals("duemilacinque", Numbers.year(2005, "it").display)
        assertEquals("duemila", Numbers.year(2000, "it").display)
        assertEquals("millenovecento", Numbers.year(1900, "it").display)
    }

    @Test
    fun clockAgreesWithTheNamedHour() {
        assertEquals("sono le due di pomeriggio", Numbers.clock(14, 0, "it").display)
        assertEquals("è l'una di pomeriggio", Numbers.clock(13, 0, "it").display)
        assertEquals("sono le due e mezza di pomeriggio", Numbers.clock(14, 30, "it").display)
        assertEquals("è l'una e un quarto di pomeriggio", Numbers.clock(13, 15, "it").display)
        assertEquals("sono le tre meno un quarto di pomeriggio", Numbers.clock(14, 45, "it").display)
        // The copula follows the hour the reading names, not the clock's hour.
        assertEquals("è l'una meno un quarto di pomeriggio", Numbers.clock(12, 45, "it").display)
        assertEquals("sono le tre meno venticinque di pomeriggio", Numbers.clock(14, 35, "it").display)
        assertEquals("sono le due e diciassette di pomeriggio", Numbers.clock(14, 17, "it").display)
    }

    @Test
    fun clockAcceptsTheTypedVariants() {
        val half = Numbers.clock(14, 30, "it").accepted
        ...
        val two = Numbers.clock(14, 0, "it").accepted
    ...
    fun theDayPartFollowsTheNamedHourAndStaysOptional() {
        assertEquals("sono le cinque di mattina", Numbers.clock(5, 0, "it").display)
        assertTrue("sono le cinque del mattino" in Numbers.clock(5, 0, "it").accepted)
        assertEquals("sono le cinque di pomeriggio", Numbers.clock(17, 0, "it").display)
        // 19:45 reads as eight o'clock, and eight is di sera.
        assertEquals("sono le otto meno un quarto di sera", Numbers.clock(19, 45, "it").display)
        // Twelve takes no part of the day: mezzogiorno and mezzanotte are its words.
        assertEquals("sono le dodici meno un quarto", Numbers.clock(11, 45, "it").display)
        assertTrue(Numbers.clock(12, 30, "it").accepted.none { " di " in it })
        ...
        for (bare in listOf("sono le cinque meno un quarto", "cinque meno un quarto", "le cinque meno un quarto")) {
            assertTrue(bare in Numbers.clock(16, 45, "it").accepted, bare)
        }
    }
    ...
    fun theMissingMinutesReadingIsAccepted() {
        ...
        assertTrue("manca un minuto alle tre" in Numbers.clock(14, 59, "it").accepted)
        // A count that apocopates does so before the noun too.
        assertTrue("mancano ventun minuti alle tre" in Numbers.clock(14, 39, "it").accepted)
    }

    /** Timetables, news and announcements run 13–23 and name no part of the day. */
    @Test
    fun theTimetableRegisterIsAccepted() {
        assertTrue("sono le quattordici e trenta" in Numbers.clock(14, 30, "it").accepted)
        assertTrue("sono le quattordici e trenta minuti" in Numbers.clock(14, 30, "it").accepted)
        assertTrue("sono le diciotto e trentacinque" in Numbers.clock(18, 35, "it").accepted)
        assertTrue("sono le ventuno" in Numbers.clock(21, 0, "it").accepted)
        assertTrue("sono le ventitré e quarantacinque" in Numbers.clock(23, 45, "it").accepted)
        ...
        assertNull(Numbers.clock(11, 0, "it").gloss)
        assertEquals("anche: sono le ventitré", Numbers.clock(23, 0, "it").gloss)
        // One o'clock stays singular: every reading of it takes "è l'", never "sono le".
        assertTrue(Numbers.clock(1, 30, "it").accepted.none { it.startsWith("sono le") })
    }

    /** A minute is counted with its noun, never with a bare "uno". */
    @Test
    fun oneMinuteIsCountedWithTheNoun() {
        assertEquals("sono le due e un minuto di pomeriggio", Numbers.clock(14, 1, "it").display)
        assertTrue("sono le quattordici e un minuto" in Numbers.clock(14, 1, "it").accepted)
        assertTrue("sono le tre meno un minuto" in Numbers.clock(14, 59, "it").accepted)
        // An apocopating count keeps its noun apocopated too.
        assertTrue("sono le due e ventun minuti" in Numbers.clock(14, 21, "it").accepted)
        assertTrue("sono le due e ventuno" in Numbers.clock(14, 21, "it").accepted)
        for (task in listOf(Numbers.clock(14, 1, "it"), Numbers.clock(14, 59, "it"))) {
    ...
    fun middayAndMidnightAreNamed() {
        val midnight = Numbers.clock(0, 0, "it")
        ...
        val noon = Numbers.clock(12, 0, "it")
        ...
        assertEquals("sono le dodici e mezza di notte", Numbers.clock(0, 30, "it").display)
    }
}
