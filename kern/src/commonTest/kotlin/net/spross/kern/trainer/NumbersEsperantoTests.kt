 ...
 */
class NumbersEsperantoTests {

    private fun display(n: Long) = Numbers.number(n, "eo").display
    private fun accepted(n: Long) = Numbers.number(n, "eo").accepted
    ...
    fun yearsAreReadAsPlainCardinals() {
        val y1978 = Numbers.year(1978, "eo")
        assertEquals("mil naŭcent sepdek ok", y1978.display)
        assertTrue("mil nauxcent sepdek ok" in y1978.accepted)
        assertEquals("du mil kvin", Numbers.year(2005, "eo").display)
        assertEquals("du mil", Numbers.year(2000, "eo").display)
        assertEquals("mil naŭcent", Numbers.year(1900, "eo").display)
    }

    /** The hour is an ordinal with `horo` elided, and it carries its own article. */
    @Test
    fun theClockNamesTheHourAsAnOrdinal() {
        assertEquals("la dua posttagmeze", Numbers.clock(14, 0, "eo").display)
        assertEquals("la unua posttagmeze", Numbers.clock(13, 0, "eo").display)
        assertEquals("la dua kaj duono posttagmeze", Numbers.clock(14, 30, "eo").display)
        assertEquals("la unua kaj kvarono posttagmeze", Numbers.clock(13, 15, "eo").display)
        assertEquals("la dua kaj dek sep posttagmeze", Numbers.clock(14, 17, "eo").display)
        assertTrue("la dua horo" in Numbers.clock(14, 0, "eo").accepted)
    }

    @Test
    fun clockAcceptsTheTypedVariants() {
        val half = Numbers.clock(14, 30, "eo").accepted
        ...
        val quarter = Numbers.clock(14, 15, "eo").accepted
        ...
        val eleven = Numbers.clock(11, 0, "eo").accepted
        assertEquals("la dek-unua antaŭtagmeze", Numbers.clock(11, 0, "eo").display)
    ...
    fun theCountdownNamesTheComingHour() {
        assertEquals("kvarono antaŭ la tria posttagmeze", Numbers.clock(14, 45, "eo").display)
        assertTrue("la tria minus kvarono" in Numbers.clock(14, 45, "eo").accepted)
        assertTrue("la dua kaj kvardek kvin" in Numbers.clock(14, 45, "eo").accepted)
        assertEquals("dek minutoj antaŭ la kvara nokte", Numbers.clock(3, 50, "eo").display)
        assertTrue("dek minutoj antaŭ la kvara matene" in Numbers.clock(3, 50, "eo").accepted)
        assertEquals("unu minuto antaŭ la tria posttagmeze", Numbers.clock(14, 59, "eo").display)
        assertTrue(Numbers.clock(14, 35, "eo").accepted.none { " minus " in it })
    }

    /** The part of the day belongs to the hour the reading NAMES, and stays optional. */
    @Test
    fun theDayPartFollowsTheNamedHour() {
        assertEquals("la kvina matene", Numbers.clock(5, 0, "eo").display)
        assertEquals("la kvina posttagmeze", Numbers.clock(17, 0, "eo").display)
        // 19:45 reads as eight in the evening while the clock still says seven.
        assertEquals("kvarono antaŭ la oka vespere", Numbers.clock(19, 45, "eo").display)
        // Counting DOWN to noon is still morning.
        assertTrue("kvarono antaŭ la dek-dua antaŭtagmeze" in Numbers.clock(11, 45, "eo").accepted)
        assertTrue(Numbers.clock(11, 45, "eo").accepted.none { "posttagmeze" in it })
        for (bare in listOf("kvarono antaŭ la kvina", "la kvara kaj kvardek kvin")) {
            assertTrue(bare in Numbers.clock(16, 45, "eo").accepted, bare)
        }
    }
    ...
    fun theTimetableRegisterIsAccepted() {
        assertTrue("la dek-kvara horo kaj tridek minutoj" in Numbers.clock(14, 30, "eo").accepted)
        assertTrue("la dek-kvara kaj tridek" in Numbers.clock(14, 30, "eo").accepted)
        assertTrue("la dek-kvara tridek" in Numbers.clock(14, 30, "eo").accepted)
        assertTrue("la dudek-unua horo" in Numbers.clock(21, 0, "eo").accepted)
        assertTrue("la nula horo kaj tridek minutoj" in Numbers.clock(0, 30, "eo").accepted)
        // Below thirteen its ordinal IS the conversational one, so only the spelled form is new.
        assertTrue("la tria horo kaj dek sep minutoj" in Numbers.clock(3, 17, "eo").accepted)
        assertTrue(Numbers.clock(14, 30, "eo").accepted.none { it.endsWith("posttagmeze") && "horo" in it })
    }

    /** One minute is counted with its noun, never with a bare numeral. */
    @Test
    fun oneMinuteIsCountedWithTheNoun() {
        assertEquals("la dua kaj unu minuto posttagmeze", Numbers.clock(14, 1, "eo").display)
        assertTrue("la dua kaj unu" in Numbers.clock(14, 1, "eo").accepted)
        assertTrue(Numbers.clock(14, 1, "eo").accepted.none { "unu minutoj" in it })
        assertTrue(Numbers.clock(14, 59, "eo").accepted.none { "unu minutoj" in it })
    }

    @Test
    fun middayAndMidnightAreNamed() {
        val midnight = Numbers.clock(0, 0, "eo")
        ...
        val noon = Numbers.clock(12, 0, "eo")
        ...
        assertEquals("la dek-dua kaj duono nokte", Numbers.clock(0, 30, "eo").display)
    }
}
