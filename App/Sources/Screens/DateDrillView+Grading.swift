...
/// The RUN half of the dates drill: how an event reaches kern, what comes back,
/// and what a close leaves behind — plus the run-through hooks that drive it.
/// State lives on DateDrillView; split out purely for file size, the way every
...
extension DateDrillView {
    ...
    func dispatch(_ intent: DateDrillIntent) {
        let reduction = DateDrillRun.shared.reduce(state: run, intent: intent, rng: drillRandom)
    ...
    private func apply(_ effect: DrillEffect) {
        ...
        case .armAdvance(let beat):
            ...
            AutoAdvance.schedule(beat.tier, &autoAdvance) {
                dispatch(DateDrillIntent.AdvanceElapsed.shared)
            }
        case .cancelAdvance:
            autoAdvance?.cancel()
        case .tone(let cue):
    ...
    /// "Finishing the reading IS the answer" — every keystroke is offered to
    /// kern, which decides whether it approves, withdraws an approval, or is
    /// ignored because a pause is standing.
    func typed() {
        dispatch(DateDrillIntent.InputChanged(text: input))
    }

    func submit() {
        dispatch(DateDrillIntent.Submit(text: input))
    }
    ...
    func checkOrReveal() {
        if input.isBlankAnswer {
            dispatch(DateDrillIntent.Reveal.shared)
        } else {
            submit()
        }
    }
    ...
    func confirm() {
        dispatch(DateDrillIntent.ConfirmPending.shared)
    }
    ...
    func closeRun() {
        ...
        onFinish(DrillRunResult(summary, title: "trainer.skill.dates"))
        dismiss()
    }

    /// The STRICT drill normalizer, built exactly as every sibling drill builds
    /// theirs: no article leniency (the pattern authors its own article, and its
    /// variants are what admit the accusative), one slip per word. Resolved
    /// once, when the run opens, for the language the answer is owed IN.
    @MainActor static func normalizer(model: AppModel, content: DateDrillContent,
                                      reverse: Bool) -> AnswerNormalizer? {
        let language = DateDrill.shared.answerLanguage(content: content, reverse: reverse)
        return model.languageInfo(language).map { AnswerNormalizer.companion.drill(answerLanguage: $0) }
    }
}

#if DEBUG
/// Run-through hooks (UserDefaults launch arguments), in the shape every
/// sibling drill uses: they drive the screen so a screenshot run needs no thumb.
extension DateDrillView {
