...
struct DateDrillView: View, LanguageNaming {
    ...
    init(model: AppModel, content: DateDrillContent, reverse: Bool, fast: Bool = false,
        ...
        #if DEBUG
        // UI-test hook: `-uitest-dates-level N` opens the run at that rung,
        // which is how the assembled rungs are reached deterministically. Kern
        // clamps it.
    ...
    /// The question on screen. A fresh calendar always has one — a rung with
    /// none is not a rung the learner could climb off.
