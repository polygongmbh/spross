 ...
 * maximum interval 365, relearning [1m]). Self-computed, NOT reference vectors —
 * they pin contract choices, not upstream numerics.
 */
class FsrsBehavioralTest {
    ...
    // Product relearning [1m]: a lapsed review card comes back after 60 s — inside
    // the session drain window (the reference's [10m] would outlive the drain loop).
    @Test
    fun productRelearningStepReturnsLapsedCardWithinDrainWindow() {
        ...
        assertEquals(60L, lapse.intervalSeconds)
        assertEquals(0, lapse.intervalDays)

        // The in-session retry a minute later graduates back to Review.
    ...
    // FSRS-6 S0(Good) = 2.3065 crosses the phrase-unlock gate (2.0) at graduation;
    // S0(Hard) = 1.2931 does not — the contract's recalibration rationale.
    @Test
    fun phraseUnlockGateCrossesAtGoodGraduation() {
        val fsrs = Fsrs(productParameters)
