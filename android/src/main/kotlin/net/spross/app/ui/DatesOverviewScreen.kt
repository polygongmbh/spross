...
import net.spross.app.dateRungHint
 ...
 * started from.
 *
 * The shape every overview uses — the run above, the reading below: the rungs a run
 ...
 * rungs, the furthest rung read and never written, Fast the single row with a price, and
 * kern sets it.
 *
 * Unlike its siblings the ladder here is not one fixed list: how tall it is depends on
 * what the pair's content carries (no `dateWithYear` pattern, no year rung) and which way
 * round the run asks (reversed, only the name rungs stand). The rows are therefore drawn
 * from kern's own [DateDrill.kinds] per rung, and the panel redraws when the reverse
...
fun DatesOverviewScreen(model: AppModel) {
    ...
    OverviewScaffold(
        ...
        OverviewPanel {
            ...
            for (rung in 1..ceiling) {
                DateRungRow(rung, DateDrill.kinds(content, rung, reverse), chrome)
            }
        }
        ...
        OverviewPanel {
            ...
            ModifierSwitchRow(
                ...
                // why: the dates rung costs THREE clean wins, so the shared "statt zwei"
                ...
                caption = if (fastOpen) {
                    ...
                    "${chrome.numbersUnlock} ${chrome.trainerRung.format(ceiling)}"
                },
                open = fastOpen,
                on = fast,
...
/**
 * One rung: what standing on it is asked. The mark is the rung's NUMBER, the letters
 ...
 */
@Composable
private fun DateRungRow(rung: Int, kinds: List<DateTaskKind>, chrome: Chrome) {
    Row(
        modifier = Modifier
            .fillMaxWidth()
        ...
        Column(Modifier.weight(1f), verticalArrangement = Arrangement.spacedBy(2.dp)) {
            Text(chrome.dateRung(kinds), style = MaterialTheme.typography.titleMedium)
            Text(
                chrome.dateRungHint(kinds),
